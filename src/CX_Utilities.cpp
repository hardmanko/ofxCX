...
float CX::Util::degreesToPixels(float degrees, float pixelsPerUnit, float viewingDistance) {
	float rad = (degrees / 2) * PI / 180;
	float length = 2 * sin(rad) * viewingDistance;
	return length * pixelsPerUnit;
}

float CX::Util::pixelsToDegrees(float pixels, float pixelsPerUnit, float viewingDistance) {
...
float CX_DegreeToPixelConverter::operator() (float deg) {
	float px = degreesToPixels(deg, _pixelsPerUnit, _viewingDistance);
	if (_roundResult) {
...
or which has inherited from that class.
...
lifetime of the coordinate converter.
*/
